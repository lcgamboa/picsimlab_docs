
\chapter{Simulator Interface}

\section{Main Window}

The main window consists of a menu, a status bar, a frequency selection combobox, an on/off button to trigger debugging, some board-specific controls and the part of the board interface itself.

In the title of the window is shown the name of the simulator PICSimLab, followed by the board and the microcontroller in use.


\begin{figure}[H]
\center
\includegraphics[width=0.99\textwidth]{img/int.png} 
\end{figure} 

The frequency selection combobox directly changes the working speed of the microcontroller.
The ``Spd'' label show the ratio between simulation speed and real time. when the ``Spd'' label goes red indicates that the computer is not being able to run the program in real time for the selected clock. 
In this case the simulation may present some difference than expected and the CPU load will be increased.

The on/off button to enable debugging is used to enable debugging support, when active simulation load is increased.

\begin{figure}[H]
\center
\includegraphics[width=0.85\textwidth]{img/int1.png} 
\end{figure} 

The first part of the status bar shows the state of the simulation, in the middle part the status of the debug support and in the last part the name of the serial port used, its default speed and the error in relation to the real speed configured in the microcontroller.

\begin{figure}[H]
\center
\includegraphics[width=0.85\textwidth]{img/int2.png} 
\end{figure} 


\subsection{Menus}

The main window menus and their functions are listed below:
\begin{itemize}
\item File
\begin{itemize}
\item Load Hex - Load .hex files
\item Reload Last - Reload the last used .hex file
\item Save Hex - Save memory in a .hex file
\item Configure - Open the \hyperlink{def:prjwizard}{Configuration} window
\item Save Workspace - Saves all current workspace settings to a .pzw file
\item Load Workspace - Loads saved settings from a .pzw file
\item Exit
\end{itemize}
\item Board
\begin{itemize}
\item xxxxx - Selects the board to be used 
\end{itemize}
\item Microcontroller
\begin{itemize}
 \item xxxxx - Selects the microcontroller to be used (depends on the selected board)
\end{itemize}
\item Modules
\begin{itemize}
\item Oscilloscope - Open the oscilloscope window
\item Spare parts - Open the spare parts window
\end{itemize}
\item Tools 
\begin{itemize}
 \item Serial Terminal - Open the serial terminal (default \hyperlink{def:sterm}{Cutecom})
 \item Serial Remote Tank - Open the \hyperlink{def:srtank}{remote tank simulator}
 \item Esp8266 Modem Simulator -  Open the \hyperlink{def:espmsim}{Esp8266 Modem Simulator}
 \item Arduino Bootloader - Load microcontroller with \hyperlink{def:aboot}{Arduino serial bootloader} 
 \item MPLABX Debugger Plugin - Open the web page to download the \hyperlink{def:mpdebug}{MPLABX Debugger Plugin} 
 \item Pin Viewer - Open the \hyperlink{def:pinv}{Pin Viewer} 
\end{itemize}
\item Code 
\begin{itemize}
 \item Project Wizard - Open the \hyperlink{def:prjwizard}{Project wizard} window 
 \item Open Active Project - Open the active project (Create or selected from Project Wizard)
 \item Open Active Project Dir - Open directory of active project (Create or selected from Project Wizard)
 \end{itemize}
\item Help 
\begin{itemize}
 \item Contents - Open the Help window
 \item Board - Open the Board Help window
 \item Examples - Load the examples
 \item About Board - Show message about author and version of board
 \item About PICSimLab - Show message about author and version of PICSimLab
 \item Check for Update - Check for new update version (stable or development)
\end{itemize}
\end{itemize}




\section{Configuration Window}\hypertarget{def:config}{}


\begin{figure}[H]
\center
\includegraphics[width=0.8\textwidth]{img/Configuration.png} 
\end{figure} 

The "PICSimLab - Configuration" window is an interface for configuring debugging and communication options.
Here's how to use each option:
\begin{itemize}
\item HWSerial: Defines the serial port to be used. The "None" option disables any connection and the 
other options listed are actual or virtual ports. This port can be used to connect the PICSimLab to other software.

\item Debug TCP Port: Defines the TCP port used for remote debugging.

\item AVR DBG: Defines the debugging protocol used for the AVR. The options are "gdb-avr" (GDB) and "microchip debugger" (MDB).

\item Remote C Port: Defines the TCP port used for communication using the remote control protocol.

\item Open Configuration Dir: Opens the directory where PICSimLab's configurations are stored.

\item Check for development version updates: Checks for available updates for the PICSimLab's development 
version instead of the stable version.

\item Save: Saves the settings.

\item Cancel: Cancels the changes and closes the window.
\end{itemize}


\section{Project Wizard}\hypertarget{def:prjwizard}{}

\subsection{Introduction and Software Requirements}

The PICSimLab Project Wizard provides a user interface for generating code projects
utilizing pre-defined code templates. To facilitate compilation and debugging,
the appropriate Integrated Development Environments (IDEs) must be installed prior
to project creation.  These IDEs are dependent on the selected boards and
microcontrollers.  For boards employing ESP32, STM32, Atmel (AVR), STM8, and C51
microcontrollers, 
the \href{https://platformio.org/install/ide?install=vscode}{PlatformIO IDE for VSCode}
is required.  Furthermore, for boards 
based on the PIC18F and PIC16F microcontrollers, 
the \hyperlink{def:installmplabx}{MPLABX IDE and the XC8 compiler}
must be installed.

\subsection{Work With Workspaces Files}

Upon saving a simulation containing a code project to a workspace file (.pzw),
the project's code is also included within the workspace file. When the workspace 
file is opened, it points to a copy of the stored code, not the original project code.
Any modifications made to the code within the opened workspace file will be automatically
saved during the file's closing.

This feature has been added to simplify student assignment submission using PICSimLab for 
simulation practice. The following steps outline its usage:
\begin{enumerate}
 \item Students begin by opening PICSimLab and utilizing the Project Wizard to
 establish a new project.
 \item Next, students will develop the code and conduct the required simulations within 
 the project.
 \item Finally, students save the workspace file and submit it for review.
 \item The instructor then receives the workspace file, containing the simulation 
 and code developed, for grading purposes.
\end{enumerate}


\subsection{Project Wizard Window}

\begin{figure}[H]
\center
\includegraphics[width=0.7\textwidth]{img/ProjectWizard.png} 
\end{figure} 

The "PICSimLab - Project Wizard" window is an interface for creating a new code project in PICSimLab. Here's how each option works:

\begin{itemize}
\item Board: Displays the name of the selected board.

\item Microcontroller: Displays the name of the microcontroller selected for the chosen board.

\item IDE: Specifies the integrated development environment (IDE) to be used for the project to be created. The options vary depending on the chosen board and microcontroller. The available options are "PlatformIO IDE for VSCode" and "MPLABX IDE".
\item Framework: Specifies the framework to be used according to the chosen board.

\item Template: Specifies the code template to be loaded according to the chosen framework.
\item Project Name: Specifies the name of your project.
\item Button Create: Creates a new project with the selected settings.

\item Button Create and Open: Creates a new project and opens the selected IDE.

\item Button Cancel: Cancels the creation of the project and closes the window.

\item Button Help: Opens this PICSimLab documentation about the ProjectWizard.

\item Button Open Existing: Opens an existing project and associates it as the active project.
\end{itemize}



\subsection{Code Templates List}

\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|c|c|c|p{8cm}|}
\hline
\textbf{IDE} & \textbf{Framework} & \textbf{Board} & \textbf{Code Template} & \textbf{Processor} \\
\hline \hline
\multirow{44}{*}{MPLAB X IDE} & \multirow{44}{*}{XC8} & \multirow{16}{*}{Breadboard}  & \multirow{11}{*}{Blink} & PIC16F1516, PIC16F1619, PIC16F1788, PIC16F1789, PIC16F1827, PIC16F1829, PIC16F18324, PIC16F1847, PIC16F18855, PIC16F1939, PIC16F1947, PIC16F628A, PIC16F648A, PIC16F688, PIC16F777, PIC16F819, PIC16F84A, PIC16F877A, PIC16F886, PIC16F887, PIC18F24Q10, PIC18F26K80, PIC18F27K40, PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F46J50, PIC18F47K40, PIC18F67J60, PIC18F67J94, atmega2560, atmega328p, attiny85\\
\cline{4-5}
 &  &   & \multirow{5}{*}{Blink FreeRTOS} & PIC18F24Q10, PIC18F26K80, PIC18F27K40, PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F46J50, PIC18F47K40, PIC18F67J60, PIC18F67J94\\
\cline{3-5}
 &  & \multirow{1}{*}{Curiosity}  & \multirow{1}{*}{Blink} & PIC16F1619\\
\cline{3-5}
 &  & \multirow{2}{*}{Curiosity\_HPC}  & \multirow{1}{*}{Blink} & PIC18F47K40\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & PIC18F47K40\\
\cline{3-5}
 &  & \multirow{1}{*}{K16F}  & \multirow{1}{*}{Blink} & PIC16F628A, PIC16F648A, PIC16F84A\\
\cline{3-5}
 &  & \multirow{1}{*}{McLab1}  & \multirow{1}{*}{Blink} & PIC16F628A, PIC16F648A, PIC16F84A\\
\cline{3-5}
 &  & \multirow{7}{*}{McLab2}  & \multirow{4}{*}{Blink} & PIC16F1789, PIC16F1939, PIC16F777, PIC16F877A, PIC16F887, PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F47K40\\
\cline{4-5}
 &  &   & \multirow{3}{*}{Blink FreeRTOS} & PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F47K40\\
\cline{3-5}
 &  & \multirow{11}{*}{PICGenios}  & \multirow{4}{*}{Blink} & PIC16F1789, PIC16F1939, PIC16F777, PIC16F877A, PIC16F887, PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F47K40\\
\cline{4-5}
 &  &   & \multirow{3}{*}{Blink FreeRTOS} & PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F47K40\\
\cline{4-5}
 &  &   & \multirow{4}{*}{Blink PICGenios} & PIC16F1789, PIC16F1939, PIC16F777, PIC16F877A, PIC16F887, PIC18F452, PIC18F4520, PIC18F4550, PIC18F4580, PIC18F45K50, PIC18F4620, PIC18F47K40\\
\cline{3-5}
 &  & \multirow{2}{*}{PQDB}  & \multirow{1}{*}{Blink} & PIC18F4520, PIC18F4550, PIC18F4620\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & PIC18F4520, PIC18F4550, PIC18F4620\\
\cline{3-5}
 &  & \multirow{2}{*}{X}  & \multirow{1}{*}{Blink} & PIC16F877A, PIC18F4550, PIC18F4620\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & PIC18F4550, PIC18F4620\\
\cline{3-5}
 &  & \multirow{1}{*}{Xpress}  & \multirow{1}{*}{Blink} & PIC16F18855\\
\cline{1-5}
\multirow{20}{*}{PlatformIO IDE for VSCode} & \multirow{16}{*}{Arduino} & \multirow{2}{*}{Arduino\_Mega}  & \multirow{1}{*}{Blink} & atmega2560\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & atmega2560\\
\cline{3-5}
 &  & \multirow{2}{*}{Arduino\_Nano}  & \multirow{1}{*}{Blink} & atmega328p\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & atmega328p\\
\cline{3-5}
 &  & \multirow{2}{*}{Arduino\_Uno}  & \multirow{1}{*}{Blink} & atmega328p\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & atmega328p\\
\cline{3-5}
 &  & \multirow{2}{*}{Blue\_Pill}  & \multirow{1}{*}{Blink} & stm32f103c8t6\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & stm32f103c8t6\\
\cline{3-5}
 &  & \multirow{2}{*}{ESP32\_C3\_DevKitC\_02}  & \multirow{1}{*}{Blink} & ESP32-C3\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & ESP32-C3\\
\cline{3-5}
 &  & \multirow{2}{*}{ESP32\_DevKitC}  & \multirow{1}{*}{Blink} & ESP32\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & ESP32\\
\cline{3-5}
 &  & \multirow{1}{*}{Franzininho\_DIY}  & \multirow{1}{*}{Blink} & attiny85\\
\cline{3-5}
 &  & \multirow{2}{*}{STM32\_H103}  & \multirow{1}{*}{Blink} & stm32f103rbt6\\
\cline{4-5}
 &  &   & \multirow{1}{*}{Blink FreeRTOS} & stm32f103rbt6\\
\cline{3-5}
 &  & \multirow{1}{*}{uCboard}  & \multirow{1}{*}{Blink} & STM8S103\\
\cline{2-5}
 & \multirow{2}{*}{CMSIS} & \multirow{1}{*}{Blue\_Pill}  & \multirow{1}{*}{Blink} & stm32f103c8t6\\
\cline{3-5}
 &  & \multirow{1}{*}{STM32\_H103}  & \multirow{1}{*}{Blink} & stm32f103rbt6\\
\cline{2-5}
 & \multirow{2}{*}{IDF} & \multirow{1}{*}{ESP32\_C3\_DevKitC\_02}  & \multirow{1}{*}{Blink} & ESP32-C3\\
\cline{3-5}
 &  & \multirow{1}{*}{ESP32\_DevKitC}  & \multirow{1}{*}{Blink} & ESP32\\
\hline
\end{tabular}
}


\section{Interaction with the Board}

On the interface area of the board it is possible to interact in some ways:

\begin{itemize}
 \item Click in ICSP connector to load an .hex file.
 \item Click in PWR button to ON/OFF the emulator..
 \item The buttons can be activated through mouse or keys 1, 2, 3 e 4.
 \item Click and drag in potentiometers to change their values.
 \item Click on EEPROM memory to view its contents.
\end{itemize}


\section{Command Line}

PICSimLab supports two command lines format:

One for load a PICSimLab Workspace file (.pzw) 
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
  picsimlab file.pzw
\end{minted}

And other for load .hex/.bin files
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
  picsimlab boardname [microcontroller [file.hex [file.pcf]]]
\end{minted}

Supported boards \textbf{boardname} parameter:
\begin{itemize}
 \item Arduino\_Mega
 \item Arduino\_Nano
 \item Arduino\_Uno
 \item Blue\_Pill
 \item Breadboard
 \item Curiosity
 \item Curiosity\_HPC
 \item ESP32\_C3\_DevKitC\_02
 \item ESP32\_DevKitC
 \item Franzininho\_DIY
 \item K16F
 \item McLab1
 \item McLab2
 \item PICGenios
 \item PQDB
 \item Remote\_TCP
 \item STM32\_H103
 \item X
 \item Xpress
 \item gpboard
 \item uCboard 
\end{itemize}



\section{Remote Control Interface}\hypertarget{def:rcontrol}{}

The remote control interface (rcontrol) allows other programs to control the PICSimLab simulation
through a TCP/IP socket using text formatted commands. 

The PICSimLab remote control interface supports TCP connections using telnet or nc (netcat).

The default port is 5000 and can be changed in configuration windows. 

The 'rlwrap' command can be used for best command edition support in telnet or nc:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
 rlwrap nc 127.0.0.1 5000
\end{minted}
  
The supported commands can be shown using the ``help'' command:  
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
 help
List of supported commands:
  blist                - list supported boards
  buclist              - list board supported MCUs
  clk [val MHz]        - show or set simulation clock
  dumpe [addr] [count] - dump internal EEPROM memory
  dumpf [addr] [count] - dump Flash memory
  dumpr [addr] [count] - dump RAM memory
  exit                 - shutdown PICSimLab
  get obj              - get object value
  help                 - show this message
  info                 - show actual setup info and objs
  loadhex file         - load hex/bin file (full path)
  oscmeasures ch       - read osc channel 1 or 2 measures
  oscrdcfg             - read osc configuration
  oscwrcfg idx "cfg"   - write osc configuration
  oscshow [0/1]        - show status or toggle osc window
  pins                 - show pins directions and values
  pinsl                - show pins formatted info
  quit                 - quit remote control interface
  reset                - reset the board
  set obj value        - set object with value
  sim [start/stop]     - show status or start/stop sim
  spadd "pname" x y    - adds the named spare part
  spdel pid/all        - delete one spare part or all
  sprdcfg pid          - read spare part configuration
  spwrcfg pid "cfg"    - write spare part configuration
  splist               - list supported spare parts
  spshow [0/1]         - show status or toggle sp window
  sync                 - wait to sync with timer event
  version              - show PICSimLab version
Ok
\end{minted}
  

The ``info'' command show all available "objects" and values:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
info
Board:     Arduino Uno
Processor: atmega328p
Frequency:   16000000 Hz
Use Spare: 1
    board.out[00] LD_L= 254
  part[00]: LEDs
    part[00].out[08] LD_1= 254
    part[00].out[09] LD_2= 30
    part[00].out[10] LD_3= 254
    part[00].out[11] LD_4= 254
    part[00].out[12] LD_5 254
    part[00].out[13] LD_6= 254
    part[00].out[14] LD_7= 254
  part[01]: Buzzer
    part[01].out[02] LD_1= 140
  part[02]: Push buttons
    part[02].in[00] PB_1= 1
    part[02].in[01] PB_2= 0
    part[02].in[02] PB_3= 1
    part[02].in[03] PB_4= 1
    part[02].in[04] PB_5= 1
    part[02].in[05] PB_6= 1
    part[02].in[06] PB_7= 1
    part[02].in[07] PB_8= 1
Ok
\end{minted}

The ``pins'' command show all pins directions and digital values:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
pins
  pin[01] ( PC6/RST) < 0                 pin[15] (  PB1/~9) > 0 
  pin[02] (   PD0/0) < 1                 pin[16] ( PB2/~10) > 0 
  pin[03] (   PD1/1) < 1                 pin[17] ( PB3/~11) > 0 
  pin[04] (   PD2/2) < 1                 pin[18] (  PB4/12) < 0 
  pin[05] (  PD3/~3) > 0                 pin[19] (  PB5/13) > 0 
  pin[06] (   PD4/4) < 1                 pin[20] (     +5V) < 1 
  pin[07] (     +5V) < 1                 pin[21] (    AREF) < 0 
  pin[08] (     GND) < 0                 pin[22] (     GND) < 0 
  pin[09] (  PB6/X1) < 0                 pin[23] (  PC0/A0) < 0 
  pin[10] (  PB7/X2) < 0                 pin[24] (  PC1/A1) < 0 
  pin[11] (  PD5/~5) < 1                 pin[25] (  PC2/A2) < 0 
  pin[12] (  PD6/~6) < 1                 pin[26] (  PC3/A3) < 0 
  pin[13] (   PD7/7) < 1                 pin[27] (  PC4/A4) > 0 
  pin[14] (   PB0/8) > 0                 pin[28] (  PC5/A5) > 0 
Ok
\end{minted}

The ``pinsl'' command show all pins info in text formatted output:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
pinsl
28 pins [atmega328p]:
  pin[01] D I 0 000 0.000 "PC6/RST " 
  pin[02] D I 1 200 0.000 "PD0/0   " 
  pin[03] D I 1 200 0.000 "PD1/1   " 
  pin[04] D I 1 200 0.000 "PD2/2   " 
  pin[05] D O 0 007 0.000 "PD3/~3  " 
  pin[06] D I 1 200 0.000 "PD4/4   " 
  pin[07] P I 1 200 0.000 "+5V     " 
  pin[08] P I 0 000 0.000 "GND     " 
  pin[09] D I 0 000 0.000 "PB6/X1  " 
  pin[10] D I 0 000 0.000 "PB7/X2  " 
  pin[11] D I 1 200 0.000 "PD5/~5  " 
  pin[12] D I 1 200 0.000 "PD6/~6  " 
  pin[13] D I 1 200 0.000 "PD7/7   " 
  pin[14] D O 0 000 0.000 "PB0/8   " 
  pin[15] D O 0 000 0.000 "PB1/~9  " 
  pin[16] D O 0 000 0.000 "PB2/~10 " 
  pin[17] D O 0 006 0.000 "PB3/~11 " 
  pin[18] D I 0 000 0.000 "PB4/12  " 
  pin[19] D O 0 000 0.000 "PB5/13  " 
  pin[20] P I 1 200 0.000 "+5V     " 
  pin[21] R I 0 000 0.000 "AREF    " 
  pin[22] P I 0 000 0.000 "GND     " 
  pin[23] A I 0 000 0.875 "PC0/A0  " 
  pin[24] A I 0 000 1.925 "PC1/A1  " 
  pin[25] A I 0 000 2.700 "PC2/A2  " 
  pin[26] A I 0 000 4.275 "PC3/A3  " 
  pin[27] D O 1 179 0.000 "PC4/A4  " 
  pin[28] D O 1 186 0.000 "PC5/A5  " 
Ok
\end{minted}


You can view one input/output state using the ``get'' command:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
get board.out[00]

get part[02].in[01]
\end{minted}

Its possible use the ``get'' command to view individual pins state:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
#digital state
get pin[19]
pin[19]= 0 
Ok

#digital mean value (0-200)
get pinm[19]
pin[18]= 100 
Ok

#analog state
get apin[25]
apin[25]= 2.700
Ok

#all info
get pinl[13]
pin[13] D I 1 200 0.000 "PD7/7   "
Ok
\end{minted}



And set value of one input using the ``set'' command:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
set part[02].in[01]  0
set part[02].in[01]  1
\end{minted}

Or set value of one pin using the ``set'' command:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
#digital
set pin[10]  2

#analog
set apin[20] 2.345
\end{minted}



For windows users \href{https://www.putty.org/}{putty telnet client} is a good option 
to access the remote control interface. 

\subsection{PICSimLab Python rcontrol Wrapper Library}

The \href{https://pypi.org/project/PICSimLab-rcontrol/}{PICSimLab python rcontrol wrapper library} is 
a Python library for controlling the PICSimLab simulator using the rcontrol remote control protocol.

They can be installed using pip:
\begin{minted}[baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{bash}
pip install PICSimlab_rcontrol
\end{minted}

Python code example:
\begin{minted}[breaklines,baselinestretch=1.2,fontsize=\footnotesize,bgcolor=colorbash]{python}
#use python -u hardware_list.py | tee hardware.md

import time,re
import subprocess
from pathlib import Path
from PICSimLab_rcontrol import PICSimLab_rcontrol


def run_picsimlab(board="", processor="", file_path=""):
    exe_path = r"/usr/bin/picsimlab"
    if len(board)  > 0: 
        if len(processor) > 0:
            if len(file_path) > 0:
                if not file_path.is_file():
                    raise  print(f"File not found {file_path}")
                return subprocess.Popen([exe_path, board, processor, str(file_path)], stdout=subprocess.DEVNULL)
            else:
                return subprocess.Popen([exe_path, board, processor], stdout=subprocess.DEVNULL)     
        else:
            return subprocess.Popen([exe_path, board], stdout=subprocess.DEVNULL)
    else:
        return subprocess.Popen(exe_path, stdout=subprocess.DEVNULL)
    
process= run_picsimlab()
try:
    time.sleep(2) 
    with PICSimLab_rcontrol(5000) as rc:
        rc.cmd_blist()
        line = rc.get_cmd_response().splitlines()[1]
        boards = [item.strip() for item in line.split(",") if item.strip()]
        rc.cmd_exit()
        process.wait()

    for board in boards:
        print(f"## {board}")
        process= run_picsimlab(board)
        time.sleep(2) 
        with PICSimLab_rcontrol(5000) as rc:
            rc.cmd_buclist()
            line = rc.get_cmd_response().splitlines()[1].split(",")
            processors = [item for item in line if item]
            print(" - Processors:",end="") 
            for proc in processors:
                print(f" {proc}",end=", ") 
            print("")   

            rc.cmd_bblist()
            line = rc.get_cmd_response().splitlines()[1].split(",")
            backends = [item for item in line if item]
            print(" - Backends:",end="") 
            for proc in backends:
                print(f" {proc}",end=", ") 
            print("")   

            rc.cmd_bdlist()
            line = rc.get_cmd_response().splitlines()[1].split(",")
            debuggers = [item for item in line if item]
            print(" - Debuggers:",end="") 
            for proc in debuggers:
                print(f" {proc}",end=", ") 
            print("")  

            rc.cmd_bilist()
            line = rc.get_cmd_response().splitlines()[1].split(",")
            ides = [item for item in line if item]
            print(" - IDEs:",end="") 
            for proc in ides:
                print(f" {proc}",end=", ") 
            print("")  

            rc.cmd_exit()
            process.wait()
                        
except ConnectionError as e:
    print(f"ConnectionError: {e}")

 
\end{minted}

\section{Picture Map Reference}\hypertarget{def:map}{}

Names used in .map files for boards and parts are standardized and used by the remote control interface. 

The names must start with \textbf{I\_} if it is an input, \textbf{O\_} if it is an output or \textbf{B\_} if it
is bidirectional. 
And be followed by one of the two-letter types in the table below before the area name. 

{%
\newcommand{\mc}[3]{\multicolumn{#1}{#2}{#3}}
\begin{center}
\begin{tabular}{cclcc}
\textbf{Function} & \textbf{Type} & \mc{1}{c}{\textbf{Description}} & \textbf{RControl In} & \textbf{RControl Out}\\
I & MD & Memory Dump & - & -\\
I & KB & Keyboard Key & 0 or 1 & 0 or 1\\
I & PG & Program & - & -\\
I & CN & Connector & - & -\\
B & PO & Potentiometer & 0 to 200 & 0 to 200\\
B & JP & Jumper & 0 or 1 & 0 or 1\\
B & VS & Value short & -32768 to 32767 & -32768 to 32767\\
B & PB & Push button & 0 or 1 & 0 or 1\\
B & DP & Dip switch & 0 or 1 & 0 or 1\\
B & SW & Switch & 0 or 1 & 0 or 1\\
B & RT & Rotary encoder & 0 to 200 & 0 to 200\\
B & AJ & Dip switch & - & -\\
O & MC & Motor Cooler & - & 0 to 200\\
O & PN & Pin name & - & -\\
O & ST & Status & - & -\\
O & IC & IC name & - & -\\
O & LR & LED RGB & - & -\\
O & LM & LED Matrix & - & -\\
O & DI & Display Info & - & -\\
O & LD & LED & - & 0 to 200\\
O & DS & Display & - & if alphanumeric show text\\
O & MT & DC motor & - & dir 0 or 1, spd. 0 to 200, pos. 0 to 200\\
O & DG & Degree & - & float angle\\
O & SS & seven segment & - & decoded number\\
\end{tabular}
\end{center}
}
 

For example area named \textbf{B\_PB\_Start}, which describes the position of a push button named "Start". 
The \textbf{B\_} bidirectional indicates that the mapped area serves as user action input and drawing output.  
 
